...
```
{
  ...
  "rationale": "The user explicitly requests correction of a written text ('corrigeme esta redaccion'). According to the important rules, any request to correct, evaluate, grade, or review work MUST select the rubric tool. The content is historical but does not require information retrieval from the available knowledge bases (recipes or math), so kb_query is not needed."
}
```
